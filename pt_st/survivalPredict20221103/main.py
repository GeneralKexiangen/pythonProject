...
if clinical == 'T1':
    ...
    clinical = [0, 0, 0, 1]
psa = st.sidebar.number_input('PSA level', min_value=0.1, max_value=98.0)
...
if gs == '7(3+4)':
    ...
    gs = [0, 0, 0, 0, 1]
nodes = st.sidebar.selectbox('Number of positive lymph nodes', ['1', '2', '3', '>3'])
if nodes == '>3':
    nodes = [1, 0, 0, 0, 0]
elif nodes == '1':
    nodes = [0, 0, 1, 0, 0]
...
clinical_data = pd.read_csv('/Users/zhiyue/PycharmProjects/pythonProject/pt_st/survivalPredict20221103/data.csv')
...
if query:
    ...
    if model == 'RSF':
        ...
        plt.title('Days to Survival Probability')
        plt.legend()
        plt.grid(True)
        st.balloons()
        st.pyplot(fig)
